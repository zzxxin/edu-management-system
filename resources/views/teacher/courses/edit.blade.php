        ...
        <form method="POST" action="{{ route('teacher.courses.update', $course) }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('year_month') is-invalid @enderror" 
                       id="year_month" name="year_month" value="{{ old('year_month', $course->year_month) }}" 
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" min="0" class="form-control @error('fee') is-invalid @enderror" 
                       id="fee" name="fee" value="{{ old('fee', $course->fee) }}" required>
                @error('fee')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
                ...
                @foreach($students as $student)
                    <div class="form-check">
                        <input class="form-check-input @error('student_ids') is-invalid @enderror" 
                               ...
                               {{ in_array($student->id, old('student_ids', $course->students->pluck('id')->toArray())) ? 'checked' : '' }}>
